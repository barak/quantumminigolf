...
QuantumSimulator::QuantumSimulator (int width, int height, float dt)
{
  ...
  psi =
    (fftwf_complex *) fftwf_malloc (sizeof (fftwf_complex) * width * height);
  prop =
    (fftwf_complex *) fftwf_malloc (sizeof (fftwf_complex) * width * height);
  xprop =
    (fftwf_complex *) fftwf_malloc (sizeof (fftwf_complex) * width * height);
...
void
QuantumSimulator::BuildMomentumPropagator ()
{
  int x, y;
  ...
  for (x = 0; x < width / 2; x++)
	...
	{
	  prop[x * height + y][0] =
	    cos (dt * (-x * x - yscale * (y - height) * (y - height)));
	  prop[x * height + y][1] =
	    sin (dt * (-x * x - yscale * (y - height) * (y - height)));
	}
    }
  for (x = width / 2; x < width; x++)
	...
	{
	  prop[x * height + y][0] =
	    cos (dt * (-(x - width) * (x - width) - yscale * y * y));
	  prop[x * height + y][1] =
	    sin (dt * (-(x - width) * (x - width) - yscale * y * y));
	}
      for (y = height / 2; y < height; y++)
	{
	  prop[x * height + y][0] =
...
void
QuantumSimulator::BuildPositionPropagator (SDL_Surface * V)
{
  int x, y;
  ...
  for (x = 0; x < width; x++)
      ...
      for (y = 0; y < height; y++)
	{
	  ...
	  xprop[x * height + y][0] =
	    cos (-.5 * (float) (red) * dt * 30000 / 255);
	  xprop[x * height + y][1] =
	    sin (-.5 * (float) (red) * dt * 30000 / 255);
	  if (red > 250)
	    {
	      xprop[x * height + y][0] = 0;
	      xprop[x * height + y][1] = 0;
...
void
QuantumSimulator::PropagateMomentum (void)
{
...
float
QuantumSimulator::PropagatePosition (float quench)
{
  float norm = 0;
...
void
QuantumSimulator::PositionMeasurement (int *x, int *y)
{
  float norm = 0;
  float psi2;
  float cutoff = 1e-6;
...
void
QuantumSimulator::GenGauss (int cx, int cy, float kx, float ky, float w)
{
  int x, y, xeff, yeff;
  float r;
...
void
QuantumSimulator::ClearWave (void)
{
  for (int x = 0; x < width; x++)
    {
      for (int y = 0; y < height; y++)
